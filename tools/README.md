...
> ⚠ 외부 repo(InstantMesh, TripoSR, gigapose, nvdiffrast, spconv, dca1000)는
> 각자 자체 README/LICENSE 를 가진 별도 프로젝트다. 아래는 **우리 프로젝트에서의
> 역할** 요약이며, 사용법 상세는 각 폴더의 README 를 참고할 것.
...
| **InstantMesh/** | 단일 이미지 → 3D 메시 | 객체 3D 생성 후보 (Trellis 대안) |
| **TripoSR/** | 단일 이미지 → 3D (빠름) | 경량 3D 생성 후보 |

## 렌더링/연산 의존성 (위 도구들이 내부적으로 사용)

| 폴더 | 정체 | 우리 역할 |
|---|---|---|
| **nvdiffrast/** | NVIDIA 미분가능 렌더러 | gigapose 의 render-and-compare 렌더링 백엔드 |
| **spconv/** | sparse convolution | 3D 네트워크(포즈/메시) 연산 의존성 |
...
> 레이더↔카메라 **외부 캘리브레이션**은 `tools/` 에서 빠졌다 → `src/utils/radar_cam_calib.py`
> 로 옮기고 `python main_r.py --calib` 으로 실행한다(경량 수신+radar+YOLO+뷰어 + yaw/baseline
...
[데스크톱] receiver → radar_live.m(레이더) / obj_crop(객체)
                         ↓ 객체 crop
                    gigapose/ (포즈) + InstantMesh|TripoSR (메시)
                         └ nvdiffrast/ spconv (렌더/연산 백엔드)
```
