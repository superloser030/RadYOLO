...
\begin{itemize}
  \item \textbf{레이더로 카메라 깊이를 미터로 고정}: 레이더가 확실히 잡은 객체의
        거리를 기준으로 단안 깊이(DA3)의 스케일을 보정한다.
  \item \textbf{역할 분담}: 레이더는 거리/속도/방위각, 카메라는 클래스/마스크/화면 위치.
        둘을 방위각으로 대응시켜 융합한다.
  \item \textbf{확장}: 융합된 거리를 이용해 객체를 3D로 배치하고, 단일 이미지 → 3D
        메시 생성으로 장면을 복원한다(2부, 진행 중).
\end{itemize}
...
[노트북] AWR1642 -> DCA1000(raw ADC) --UDP--> [데스크톱] receiver
         웹캠(MJPG)                  --UDP-->            receiver
                                                          |
   receiver --(raw ADC)--> MATLAB(radar_live.m, CFAR) -> targets.json
   receiver --(latest frame)--> YOLO11x-seg / DepthAnythingV3
                                                          |
                              radar_fusion (방위각 매칭 + 스케일 보정)
                                                          |
                          live_overlay.json  ->  웹 뷰어(2D/3D)
\end{lstlisting}
...
RadYOLO/
  ...
  src/
    ...
    viewer/              # viewer.html (2D/3D 통합 뷰어)
  matlab/                # radar_live.m, cfarDetect.m (CFAR 신호처리)
  workflows_comfyui/     # DA3/ESRGAN ComfyUI 워크플로 JSON
  config/                # network.toml, sender.toml, receiver.toml
  tools/                 # dca1000(캡처 CLI), gigapose(외부 repo)
  data/                  # 생성물(scene/objects/radar/webcam) — gitignore
\end{lstlisting}
